...
* Discrete-Event Simulation Structure 1/3

All discrete-event simulators share a common architecture

** Event Scheduler                                                 :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Maintains a linked list of events waiting to occur

Key manipulation activities:
- Schedule event X at time T
- Hold event X for time interval dt
- Cancel a previously scheduled event
- Hold event indefinitely
- Schedule an indefinitely held event

Most frequently executed component \to runs before every event

#+latex: \pause

** Simulation Clock & Time-Advancing                               :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:
...
Two approaches:
- *Unit time approach*: Increments by small steps, checks for events
- *Event-driven approach*: Jumps directly to next event time

Event-driven approach is standard for computer simulations

* Discrete-Event Simulation Structure 2/3
** Initialization Routines                                         :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Set initial state of system variables

Initialize random-number generation streams

#+latex: \pause

** System State Variables                                          :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Global variables describing system state

Examples
- Number of jobs in CPU scheduling queue
- Number of flows in a communication channel
- Number of concurrent actors using a specific resource

#+latex: \pause

** Event Routines                                                  :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Each event has its own routine

Functions:
- Update system state variables
- Schedule other events

Example: CPU scheduling needs routines for job arrivals, sched., and departures

* Discrete-Event Simulation Structure 3/3
** Report Generator                                                :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Output routines executed at simulation end

Calculate and print final results in specified format

** Trace Routines                                                  :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

Print intermediate variables during execution

Essential for debugging simulation programs

Features:
- On/off toggle for production runs
- Optional keyboard interrupt capability

#+latex: \pause

** Main Program                                                    :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:
...
Workflow:
1. Call input routines
2. Initialize simulation
3. Execute iterations
4. Call output routines
